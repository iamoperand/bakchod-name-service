...
contract Domains is ERC721URIStorage {
  ...
  constructor(string memory _tld) payable ERC721("Bakchod Name Service", "BNS") {
  ...
  function price(string calldata name) public pure returns(uint) {
    ...
    require(len > 0, "name must be non-empty");
  ...
  // Register a domain name against caller's address 
  function register(string calldata name) public payable {
    // check that the name is not already registered
      ...
      bytes(
        string(
          abi.encodePacked(
            '{"name": "',
            _name,
            '", "description": "A domain on the Bakchod Name Service", "image": "data:image/svg+xml;base64,',
    ...
    _tokenIds.increment();
    console.log("%s has registered \"%s\" domain", msg.sender, name);
  }
  ...
  function setRecord(string calldata name, string calldata record) public {
    ...
    records[name] = record;
    console.log("%s has set record for \"%s\" domain", msg.sender, name);
  }
